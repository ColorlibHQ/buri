...
// social media
if ( ! function_exists( 'buri_social' ) ) {
	function buri_social( $args = array()  ){
...
function buri_contact_form7_shortcode(){

    // contact form list
    // $getforms['cs'] = __( 'Custom Shortcode', 'buri' );
    $getforms = [];
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'buri' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'buri' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function buri_get_post_views($postID){
    $count_key = 'buri_post_views_count';
...
function buri_set_post_views($postID) {
    $count_key = 'buri_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
...
// blog post categoty 
function buri_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'buri' ) );
